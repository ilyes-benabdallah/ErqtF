...
function ajouterAdmin($pseudoAdmin,$nomAdmin,$prenomAdmin,$emailAdmin,$passAdmin,$confirmeAdmin)
{
	$objectErreur = new myClassErreur();
	if(empty ($pseudoAdmin) or empty ($nomAdmin) or empty ($prenomAdmin) or empty ($emailAdmin) or empty ($passAdmin)or empty ($confirmeAdmin))
	{
	...
	elseif ($passAdmin != $confirmeAdmin)
	{
		...
		{
			$insertAdmin= mysql_query("INSERT INTO admin (nom_admin, prenom_admin, pseudo_admin, password_admin, email_admin) VALUE ('$nomAdmin','$prenomAdmin', '$pseudoAdmin','$passAdmin', '$emailAdmin')") or die(mysql_error());
...
function supprimerAdmin($idAdmin)
{
	...
	if ($requetteSuppAdmin) 
					...
					//echo "<script>window.location.replace('afficherAdmins.php#admin');</script>";
					exit;
					}
...
function modifierPass($idAdmin,$actualPass,$nouveauPass,$confirmPass)
{
		...
		elseIf (md5($actualPass) != $_SESSION["PASS_ADMIN_VAL"])
		{
